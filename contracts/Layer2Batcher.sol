...
contract Layer2Batcher is BaseChildTunnel, Ownable, Initializable {
    ...
    event DepositBatch(uint amount, uint[][] depositAmounts);
    event SellTokenBatch(uint[][] tokenAmounts);
    ...
    function deposit(uint amount, uint detf) virtual public {
        ...
        if (!includes(batch.users, msg.sender)) {
    ...
    /**
     *  @dev Check `value` is included in `users` array
     *  @param users array of addresses
     *  @param value address
     *  @return bool
     */
    function includes(address[] memory users, address value)
        public
        pure
        returns(bool)
    {
    ...
    function executeDepositBatch() virtual public {
        ...
        Batch storage batch = depositBatches[depositBatchId];
        uint[][] memory data = new uint[][](detfs.length);
        uint depositSum = 0;

        batch.status = BatchLifeCycle.Fired;
        batch.amountsL2 = new uint[](detfs.length);
        ...
        for (uint i = 0; i < detfs.length; i++) {
            data[i] = new uint[](batch.users.length);
            tunnelData.amounts[i] = 0;

            for (uint j = 0; j < batch.users.length; j++) {
                address user = batch.users[j];
                uint amount = batch.amounts[user][i];

                tunnelData.amounts[i] += amount;
                data[i][j] = amount;
            }

            batch.amountsL2[i] = tunnelData.amounts[i];
            depositSum += tunnelData.amounts[i];
        }
        ...
        emit DepositBatch(depositSum, data);
    }
    ...
    function executeSellBatch() virtual public {
        TunnelData memory tunnelData;
        Batch storage batch = sellBatches[sellBatchId];
        uint[][] memory data = new uint[][](erc20Tokens.length);

        batch.status = BatchLifeCycle.Fired;
        batch.amountsL2 = new uint[](erc20Tokens.length);
